...
const IngresoInstrumentosScreen = () => {
  // --- ESTADOS ---
  ...
  const [isAddInstrumentModalOpen, setIsAddInstrumentModalOpen] = useState(false); // NUEVO: Estado para Agregar Instrumentos
  ...
  // --- DESCARGA ---
  const handleDownload = () => { /* Lógica de descarga igual */ };
  ...
  const ModalInput = ({ label, placeholder, icon: Icon, defaultValue, className="w-full", readOnly=false }) => (
    ...
    <div className="space-y-6 h-full flex flex-col font-sans relative" onClick={() => setOpenMenuIndex(null)}>
      ...
      <div className="flex flex-col xl:flex-row gap-4 justify-between items-center bg-white p-1 rounded-xl">
        ...
        <div className="flex items-center gap-4">
            <button onClick={(e) => { e.stopPropagation(); setIsCreateModalOpen(true); }} className="flex items-center gap-1 text-blue-500 font-medium text-xs hover:text-blue-600 transition-colors">
                Crear entrega de instrumentos de 3ros <PlusCircle size={16} />
            </button>
            <button onClick={(e) => { e.stopPropagation(); handleDownload(); }} className="flex items-center gap-1 text-blue-500 font-medium text-xs hover:text-blue-600 transition-colors border-l border-slate-200 pl-4">
              ...
              {data.map((row, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors text-slate-500">
                  ...
                  <td className="px-4 py-4 text-right relative">
                    <button onClick={(e) => toggleMenu(i, e)} className={`p-1 rounded-full transition-colors ${openMenuIndex === i ? 'bg-blue-50 text-blue-500' : 'text-slate-300 hover:text-blue-500 hover:bg-slate-50'}`}><MoreVertical size={18} /></button>
                    {openMenuIndex === i && (
