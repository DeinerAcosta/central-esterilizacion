...
const RegistroEsterilizacionScreen = () => {
  // --- ESTADOS ---
  const [isModalOpen, setIsModalOpen] = useState(false); // Modal Crear (Mantenemos el existente)
  const [isEditModalOpen, setIsEditModalOpen] = useState(false); // NUEVO: Modal Editar
  const [editingItem, setEditingItem] = useState(null); // NUEVO: Datos del ítem a editar
  ...
  const ModalInput = ({ label, placeholder = "", icon: Icon, required = false, type = "text", isSelect = false, className = "" }) => (
