...
const KitScreen = () => {
  // --- ESTADOS GLOBALES ---
  const [view, setView] = useState('list'); // 'list', 'create', 'edit', 'details' (NUEVO)
  const [isAlertOpen, setIsAlertOpen] = useState(false); // Alerta duplicados
  const [isStatusAlertOpen, setIsStatusAlertOpen] = useState(false); // Alerta cambio estado
  const [itemToToggle, setItemToToggle] = useState(null); // Item a cambiar estado
  const [selectedKit, setSelectedKit] = useState(null); // Kit seleccionado para ver/editar
  ...
  const CodigoKitBlock = ({ visible = true }) => (
