...
const DashboardScreen = () => {
  ...
  // 1. Datos Box Plot Simulado
  const dataTimeProcess = [
  ...
  // 6. NUEVO: Datos Consumo Insumos (Gráfico de Área Inferior)
  const dataConsumption = [
    ...
    { month: 'Jun', current: 210, previous: 200 }, // Punto alto
  ...
  // Simulación visual del "Box Plot" (Barra degradada con línea vertical)
  const CustomBoxBar = (props) => {
    const { x, y, width, height, fill } = props;
    const radius = 4;
    return (
      <g>
        {/* Línea gris del rango (Bigote) */}
        <line x1={x + width/2} y1={y - 15} x2={x + width/2} y2={y + height + 15} stroke="#cbd5e1" strokeWidth={2} strokeLinecap="round" />
        {/* Barra del cuerpo (Box) */}
        <rect x={x} y={y} width={width} height={height} fill={fill} rx={radius} ry={radius} />
        {/* Triángulos decorativos extremos (opcional para exactitud pixel-perfect) */}
        <path d={`M${x + width/2 - 4},${y - 15} L${x + width/2 + 4},${y - 15} L${x + width/2},${y - 10} Z`} fill="#cbd5e1"/>
        <path d={`M${x + width/2 - 4},${y + height + 15} L${x + width/2 + 4},${y + height + 15} L${x + width/2},${y + height + 10} Z`} fill="#cbd5e1"/>
      </g>
    );
  };
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 bg-white p-6 rounded-3xl shadow-soft border border-slate-100 flex flex-col">
          <div className="flex justify-between items-start mb-4">
            <h3 className="font-bold text-slate-700 text-sm leading-tight">Promedio de Instrumentos<br/>rechazados</h3>
        ...
        <div className="lg:col-span-3 bg-white p-6 rounded-3xl shadow-soft border border-slate-100">
          <h3 className="font-bold text-slate-700 text-sm mb-4">Utilización de Kit</h3>
          <div className="space-y-4">
            {kitsData.map((kit, i) => (
              <div key={i} className="flex justify-between items-center pb-3 border-b border-slate-50 last:border-0">
                <div>
                  <div className="text-[10px] text-slate-400 uppercase tracking-wide font-medium">{kit.name.split('-')[0]}</div>
                  <div className="text-sm font-bold text-blue-600">{kit.name.split('-')[1]}</div>
                </div>
                <span className={`text-xs font-bold flex items-center gap-1 ${kit.status === 'up' ? 'text-emerald-500' : 'text-rose-500'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-3 bg-white p-6 rounded-3xl shadow-soft border border-slate-100 relative flex flex-col items-center">
              ...
              <PieChart>
                ...
                >
                  {dataCycles.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={COLORS_CYCLES[index % COLORS_CYCLES.length]} cornerRadius={10} />
                  ))}
                </Pie>
              </PieChart>
            </ResponsiveContainer>
          ...
          <div className="flex gap-4 justify-center w-full mt-2 text-[10px] font-medium text-slate-500">
             <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded bg-emerald-500"></div>Exitosos</div>
             <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded bg-slate-200"></div>Fallidos</div>
          </div>
        </div>
        ...
        <div className="lg:col-span-6 bg-white p-6 rounded-3xl shadow-soft border border-slate-100 flex flex-col">
          <div className="flex justify-between items-center mb-6">
            ...
            <button className="border border-slate-200 rounded-lg px-3 py-1.5 flex items-center gap-2 text-slate-500 text-xs hover:bg-slate-50 transition-colors">
      ...
      {/* ================= FILA 3: CONSUMOS (NUEVO) ================= */}
      <div className="bg-white p-6 rounded-3xl shadow-soft border border-slate-100">
         {/* Header con Filtros */}
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <h3 className="font-bold text-slate-700 text-sm">Promedio de Consumos de Insumos Quirúrgicos</h3>
            ...
            <div className="flex flex-wrap items-center gap-3">
               ...
               <div className="flex gap-2">
                 {['Año', 'Tipo de insumo', 'Proveedor'].map((label, i) => (
                    <button key={i} className="flex items-center gap-2 px-3 py-1.5 border border-slate-200 rounded-full text-xs text-slate-500 hover:border-blue-300 bg-white transition-colors">
         ...
         {/* Gráfico de Área (Curvas) */}
         <div className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={dataConsumption} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <defs>
                  {/* Gradiente Azul Brillante para Año Actual */}
                ...
                {/* Año Actual (Azul con Gradiente y Punto destacado) */}
                <Area 
                   type="monotone" 
                   dataKey="current" 
                   stroke="#0ea5e9" 
                ...
                {/* Punto destacado estático en Junio (simulado visualmente igual a la imagen) */}
                {/* Nota: En una app real esto sería dinámico o un CustomDot */}
              </AreaChart>
            </ResponsiveContainer>
         </div>
      </div>
