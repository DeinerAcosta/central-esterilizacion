...
const HojasDeVidaScreen = () => {
  const [activeTab, setActiveTab] = useState('hv'); 
  const [isModalOpen, setIsModalOpen] = useState(false); // Modal Crear HV
  const [isTrasladarOpen, setIsTrasladarOpen] = useState(false); // Modal Trasladar
  const [compatible, setCompatible] = useState(false);
  const [transferType, setTransferType] = useState('kit');
  ...
  // Componentes Helper
  const FloatingLabelSelect = ({ label, required = false, defaultValue = "Seleccionar" }) => (
  ...
  const FloatingLabelInput = ({ label, required = false, type = "text", placeholder = "" }) => (
    <div className="relative w-full">
        <label className="absolute -top-2.5 left-4 bg-white px-2 text-xs text-slate-500 font-medium z-10">
            {label}{required && <span className="text-red-500 ml-0.5">*</span>}
        </label>
